...
export default (props: {instrument: Sampler, canvasWidth: number}) => {
  ...
  const getVisibleTime = createMemo(() => {
    ...
    const visibleWaveformLength = (waveform.length - props.instrument.navigation.start - navigation.end);
  ...
  const additionalOffset = createMemo(()=> {
    ...
    const navigationStartOffset = ((navigationStartTime * spb()) % 1) * gridWidth;
