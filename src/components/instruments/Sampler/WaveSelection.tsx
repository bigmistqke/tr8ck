...
export default (props: { canvasWidth: number; instrument: Sampler }) => {
  ...
  const calculateOffset = (type: "start" | "end") => {
    ...
    const value =
      ((props.instrument.navigation.start - props.instrument.selection[type]) *
        props.canvasWidth) /
      (props.instrument.waveform.length -
        props.instrument.navigation.start -
        props.instrument.navigation.end);
