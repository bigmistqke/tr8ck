...
export default () => {
  ...
  return (
    <>
      <div class="h-48 flex">
        <Block extraClass="relative flex-1 flex overflow-hidden">
          <Show
            when={!filesOpened()}
            fallback={
              <SamplerList input={input!} setFilesOpened={setFilesOpened} />
            }
          >
      ...
      <div class="flex gap-2">
        ...
        <ButtonBar
          onclick={() => {
            if (store.arrayBuffers.length > 0) setFilesOpened(bool => !bool);
            else input.click();
          }}
        >
          load
        ...
        <SliderBar
          onupdate={value =>
            actions.setSamplerSpeed(speed => speed + value / 100)
          }
        >
