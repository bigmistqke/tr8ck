...
export default (props: {
  ...
  const getName = () =>
    "dspMeta" in props.state?.node
      ? props.state.node.dspMeta.name
      : props.state.initialName;

  const dragstart = () => {
    if (props.draggable && !props.factory) setDraggable(true);
  ...
  return (
    <Show
      when={
        props.state.initialName !== "input" &&
        props.state.initialName !== "output"
      }
    >
      <Block
      ...
      >
        <div
          class={`flex flex-col flex-1 ${
            !props.disableOnOff &&
            (props.factory || draggable() || store.dragging.fx)
              ? "pointer-events-none"
              : ""
          }`}
        >
          <div
            class={`flex ${props.draggable ? "cursor-move" : ""}`}
            ...
            <For each={props.state.parameters}>
              {parameter => (
                <FxParameter
                  parameter={parameter}
                  node={
                    "factoryId" in props.state ? props.state.node : undefined
                  }
                />
              )}
            </For>
