...
export default (props: {
  ...
  const update = (delta: number) => {
    ...
    if (
      !(props.node === track.pitchshifter && props.parameter.label === "shift")
    ) {
      props.node.setParamValue(props.parameter.address, value);
    } else {
      props.node.setParamValue(
        props.parameter.address,
        value + track.semitones
      );
    }
  };
  const getRotation = () =>
    ((props.parameter.value - props.parameter.min) /
      (props.parameter.max - props.parameter.min)) *
      180 -
    90;
