  ...
  return (
    <div class="flex-1 flex flex-col w-96 relative">
      <div class="absolute flex flex-col z-10 w-full h-full">
        <div
          class="flex gap-2 pb-0 p-2 overflow-auto "
          style={{
        ...
        >
          <Bar
            ...
            style={{
              background: actions.getPatternColor(store.selection.patternId),
            }}
          >
            #
            {store.patterns.findIndex(
              pattern => pattern.id === store.selection.patternId
            )}
          </Bar>
          <ButtonBar onclick={actions.duplicateSelectedPattern}>
            clone
          </ButtonBar>
          <ButtonBar onclick={actions.clearSelectedPattern}>clear</ButtonBar>
          <ButtonBar onclick={actions.incrementSelectedPatternId}>
            tempo
          </ButtonBar>
        </div>
        <div
          class="pl-2 pr-2 mt-1 mb-2 flex-1 flex-col gap-2 overflow-auto"
          style={{
            "scrollbar-gutter": "stable",
          }}
        >
          <div class="flex-1 flex gap-2 min-h-full">
            <Index
              each={Array(actions.getSelectedPattern()!.sequences.length).fill(
                0
              )}
            >
              {(_, index) => (
                <Track
                  amount={
                    actions.getSelectedPattern()!.sequences![index].length
                  }
        ...
        <div class="p-2 pt-0 ">
          <div
            class="flex gap-2 w-full h-4 mb-2 overflow-hidden pr-6"
            style={{
              "scrollbar-gutter": "stable",
            }}
          >
