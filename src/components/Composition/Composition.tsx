...
import { CompositionGroupProps, CompositionElementProps } from "../../types";
...
export default () => {
    const dragover = (e:DragEvent) => {
      ...
      actions.dragComposition(targetId!, drag.id, drag.patternId, position)
    }
    ...
    return (
      <Block extraClass="flex flex-0 w-64 p-2 bg-neutral-100 max-h-1/2 overflow-hidden">
                ...
                <div class="flex flex-1 overflow-hidden rounded-xl">
                  <div class="flex-1 overflow-auto bg-white p-2">
                    <For each={store.patterns} >
                      {
                        (pattern, i) => <PatternElement patternId={pattern.id} index={i()}/>
                      }
                    </For>
                  </div>
                </div>
