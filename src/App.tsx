...
function App() {
  const setSelectedColorCSS = () => {
    ...
    if ("color" in instrument) {
      root.style.setProperty(
        "--selected-instrument",
        (instrument as Instrument).color
      );
    }
  ...
  const initApp = async () => {
    ...
    root.style.setProperty(
      "--selected-instrument",
      actions.getSelectedInstrument().color
    );
  ...
  return (
    <>
      <For each={store.editors}>{editor => <EditorModal {...editor} />}</For>
              ...
              <Bar extraClass="bg-white flex gap-2 transition-colors">
                <button
                  class={`h-4 hover:text-red-500 transition-colors ${
                    store.audioRecorder &&
                    store.audioRecorder.type === "resample"
                      ? "animate-record"
                      : ""
                  }`}
                ...
                <button
                  class={`h-4 hover:text-red-500 transition-colors ${
                    store.audioRecorder && store.audioRecorder.type === "file"
                      ? "animate-record"
                      : ""
                  }`}
                ...
                <button
                  class={`h-4 hover:text-red-500 transition-colors ${
                    store.audioRecorder && store.audioRecorder.type === "mic"
                      ? "animate-record"
                      : ""
                  }`}
                ...
                >
                  <Show
                    when={!store.bools.playing}
                    fallback={<TiMediaPause class="h-4 w-4" />}
                  >
